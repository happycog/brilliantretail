...
class Gateway_payleap extends Brilliant_retail_gateway {
	// Required variables
	public $title 	= 'Payleap';
	public $label 	= 'Credit Card Payment (Payleap)';
	public $descr 	= 'Accept credit cards directly from your site with a Payleap gateway account.';
	public $enabled = true;
	public $version = 1.0;
	...
	function process($data,$config)
	{
    	...
    	$args .= "&ExtData=<TrainingMode>".substr($config['test_mode'],0,1)."</TrainingMode><Invoice><InvNum>".$orderid."</InvNum><BillTo><Name>".$data['br_billing_fname']." ".$data['br_billing_lname']."</Name><Address><Street>".$data['br_billing_address1']."</Street><City>".$data['br_billing_city']."</City>
	<State>".$data['br_billing_state']."</State><Zip>".$data['br_billing_zip']."</Zip><Country>".$data['br_billing_country']."</Country></Address><Email>".$email."</Email><Phone>".$data['br_billing_phone']."</Phone></BillTo><Description>".$config['order_description']."</Description></Invoice>";
		$args .= "&PNRef=";
    	$args .= "&MagData="; 
