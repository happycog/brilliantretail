<?php
	...
	*/
	...
	class IdealRequest
	{
		...
		var $sPrivateKeyPass;
		var $sPrivateKeyFile;
		var $sPrivateCertificateFile;
		var $sPublicCertificateFile;

		// Account settings
		var $bABNAMRO = false; // ABN has some issues
		var $sAquirerName;
		var $sAquirerUrl;
		var $bTestMode = false;
		var $sMerchantId;
		...
		function setPrivateKey($sPrivateKeyPass, $sPrivateKeyFile = false, $sPrivateCertificateFile = false)
		{
		...
		function setMerchant($sMerchantId, $sSubId = 0)
		{
		...
		function setAquirer($sAquirerName, $bTestMode = false)
		{
		...
		function checkConfiguration($aSettings = array('sSecurePath', 'sPrivateKeyPass', 'sPrivateKeyFile', 'sPrivateCertificateFile', 'sPublicCertificateFile', 'sAquirerUrl', 'sMerchantId'))
		{
			$bOk = true;
		...
		// Security functions
		function getCertificateFingerprint($bPublicCertificate = false)
		{
		...
		function getSignature($sMessage)
		{
			if($fp = fopen($this->sSecurePath . $this->sPrivateKeyFile, 'r'))
			{
				...
				{
					$this->setError('Invalid password for private key file: ' . $this->sPrivateKeyFile, false, __FILE__, __LINE__);
				}
		...
		function verifySignature($sData, $sSignature)
		{
			$bOk = false;
					...
					else
					{
						...
						while(strpos($sXmlReply, '<issuerID>'))
						{
							...
							if(strcmp($sIssuerList, 'Short') === 0) // Short list
							{
								...
								// if((strcasecmp(substr($_SERVER['SERVER_PROTOCOL'], 0, 5), 'HTTPS') === 0) || (strpos($sIssuerName, 'ABN') === false))
								{
	...
	class TransactionRequest extends IdealRequest
	{
		...
		function setOrderAmount($fOrderAmount)
		{
		...
		function setIssuerId($sIssuerId)
		{
			$this->sIssuerId = $sIssuerId;
		}

		// A random generated entrance code
		function setEntranceCode($sEntranceCode)
		{
	...
	class StatusRequest extends IdealRequest
	{
		...
		// Set transaction id
		function setTransactionId($sTransactionId)
		{
